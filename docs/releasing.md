...
5. Create and push an annotated `vVERSION` tag on the tested commit. Publish a GitHub release using the corresponding notes and attach the source archive, Arch package, `PKGBUILD`, `.SRCINFO` and `SHA256SUMS`.
...
The generated source recipe is ready for a future AUR submission. Copy its `PKGBUILD` and `.SRCINFO` into the AUR package repository when an account is available. Do not advertise an AUR install command until the package is actually published there.
